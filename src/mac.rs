...
const TAG: &str = "mac.c";
static mut tx: *mut tx_func = null_mut();
...
unsafe extern "C" fn open_mac_rx_callback(packet: *mut wifi_promiscuous_pkt_t) {
    let p: *mut mac80211_frame = packet.as_mut().unwrap().payload;

    // fuck beacon frames, all my homies hate beacon frames
    // Oh dear, what am I in for
    if p.as_mut().unwrap().frame_control.typ == IEEE80211_TYPE_MGT
    ...
    if (memcmp(
        module_mac_addr.as_ptr(),
        p.as_mut().unwrap().receiver_address,
        6,
    )) && (memcmp(BROADCAST_MAC, p.as_mut().unwrap().receiver_address, 6))
    {
    ...
    error!(
        ...
        MAC2STR!(p.as_mut().unwrap().transmitter_address),
        MAC2STR!(p.as_mut().unwrap().receiver_address),
        p.as_mut().unwrap().frame_control.typ,
    ...
    loop {
        ...
        if xQueueReceive(reception_queue, packet.as_mut_ptr().cast(), 10) != 0 {
            ...
            match sta_state {
                ...
                {
                    if p.as_mut().unwrap().frame_control.typ == IEEE80211_TYPE_MGT
                        && p.frame_control.sub_type == IEEE80211_TYPE_MGT_SUBTYPE_AUTHENTICATION
                    {
                ...
                {
                    if p.frame_control.type_() == IEEE80211_TYPE_MGT
                        && p.frame_control.sub_type() == IEEE80211_TYPE_MGT_SUBTYPE_ASSOCIATION_RESP
                    {
            ...
            esp_idf_svc::sys::free(packet.cast());
        }
        ...
        match sta_state {
            IDLE => {
                ...
                tx(to_ap_auth_frame, size_of::<to_ap_auth_frame>());
            }
            AUTHENTICATED => {
                error!("{}: Sending association request frame!", TAG);
                tx(to_ap_assoc_frame, size_of::<to_ap_assoc_frame>());
            }
            ASSOCIATED => {
                error!("{}: Sending data frame", TAG);
                tx(to_ap_data_frame, size_of::<to_ap_data_frame>());
            }
            _ => {}
        }
